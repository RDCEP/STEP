...
def show_save_intensities(data: np.ndarray, colormap: LinearSegmentedColormap, title: str, unit: str, start_time=0,
    ...
    By default, images will be shown and saved; see start_time documentation for information on how to show or save
    images only.
    ...
    for time_index in range(num_time_slices):
        ...
        CB.set_ticks([minimum, maximum * .25, maximum * .5, maximum * .75, maximum])
...
def show_save_storms(data: np.ndarray, colors: LinearSegmentedColormap or list, title: str, start_time=0,
    ...
    can be shown and saved, and the GIF can only be saved. By default, images will be shown and saved; see start_time
    ...
    :param ticks: the toggle for including ticks in the plot, given as a boolean. The default value is False.
    :return: (None.)
    """
    # close all plots open for display
    plt.close('all')
...
def histogram(data: np.ndarray, bins: int, bin_range: tuple) -> None:
    ...
    :param bin_range: the lower and upper range of the bins.
    :return: (None - the resulting histogram is only plotted.)
    """
    ...
    plt.title('Frequency of precipitation intensities')
